...
exec(`gcloud beta billing projects link ${app_conf.projectID} --billing-account=${billingAccountID}`)
...
exec(`gsutil mb -p ${app_conf.projectID} -c standard -l ${app_conf.region} -b on gs://${app_conf.project_name}-storage-bucket/`)
...
const instanceID = `${app_conf.project_name}-sql-instance5`
console.log('Creating sql instance, this may take a few minutes...')
exec(`gcloud sql instances create ${app_conf.instanceID} --tier=db-n1-standard-1 --app_conf.region=${app_conf.region}`)
// set root password
console.log('Setting root password')
exec(`gcloud sql users set-password root --host % --instance ${app_conf.instanceID} --password ${app_conf.sqlRootPassword}`)
// create user with password
console.log(`Creating sql instance user account: ${app_conf.mySqlUsername}`)
exec(`gcloud sql users create ${app_conf.mySqlUsername} --host % --instance ${app_conf.instanceID} --password ${app_conf.sqlUserPassword}`)
////// Assign user permissions to tables


////// create mySQL Database
exec(`gcloud sql databases create ${app_conf.project_name}-db --instance ${app_conf.instanceID}`)
// authorize IP range
// assign authorized network 0.0.0.0/0
console.log('Assigning 0.0.0.0/0 as authorized external network')
exec(`gcloud sql instances patch ${app_conf.instanceID} --authorized-networks=0.0.0.0/0 --quiet`)
...
//// create app engine
exec(`gcloud app create --app_conf.region ${app_conf.region}`)
