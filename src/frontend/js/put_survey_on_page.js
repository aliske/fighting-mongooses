async function putSurveyOnPage() {
    ...
    for (var i = 0; i < questions.length; i++) {
        ...
        if (questions[i].type == "Short Answer") {
            ...
            table_data += "<fieldset><legend>Write a short response:</legend>";
            ...
            table_data += "<fieldset><legend>Choose one:</legend>";
            for (const option of options[i]) {
                table_data += `<input type='radio' name='question-${questions[i]['id']}' value='${option['option_value']}'> ${option['option_value']}<br>`
            }
            table_data += "</fieldset>";
            ...
            table_data += "<fieldset><legend>Choose one or more:</legend>";
            for (const option of options[i]) {
                table_data += `<input type='checkbox' name='question-${questions[i]['id']}' value='${option['option_value']}'> ${option['option_value']}<br>`
            }
            table_data += "</fieldset>";
